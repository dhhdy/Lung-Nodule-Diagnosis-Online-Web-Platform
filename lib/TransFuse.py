...
from lib.DeiT import deit_base_patch16_224 as deit_base
from lib.DeiT import deit_base_patch16_384 as deit_base_384
...
class TransFuse_S(nn.Module):
    def __init__(self, in_class=1, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        super(TransFuse_S, self).__init__()

        self.resnet = resnet34()
        self.resnet.conv1 = nn.Conv2d(in_class, 64, kernel_size=7, stride=2, padding=3,bias=False)
        if pretrained:
            dic = torch.load('/root/autodl-tmp/pretrained/resnet34.pth')
            del dic['conv1.weight']
        if pretrained:
