...
class UNet(nn.Module):
    def __init__(self,
        ...
        block_size = [96, 192, 384, 768, 1436]
        ...
        self.up44 = Up(dim//8, dim//16)
        # 分支融合
        self.ddfs1 = Ddfusion(in1=dim  // 2, in2=dim // 2, h=size//8, w=size//8, rate=2)
        self.ddfs2 = Ddfusion(in1=dim//4, in2=dim//4, h=size//4, w=size//4, rate=2)
        self.ddfs3 = Ddfusion(in1=dim//8, in2=dim//8, h=size//2, w=size//2, rate=2)
        self.ddfs4 = Ddfusion(in1=dim//16, in2=dim//16, h=size, w=size, rate=2)
        ...
        # self.C1 = DoubleConv(block_size[0] * 2, block_size[0])
        # self.C2 = DoubleConv(block_size[1] * 2, block_size[1])
        # self.C3 = DoubleConv(block_size[0] * 3, block_size[0])
        self.C4 = DoubleConv(block_size[2] * 2, block_size[2])
        # self.C5 = DoubleConv(block_size[1] * 3, block_size[1])
        self.C55 = DoubleConv(block_size[1] * 2, block_size[1])
        # self.C6 = DoubleConv(block_size[0] * 4, block_size[0])
        self.C66 = DoubleConv(block_size[0]*2,block_size[0])
        # self.C7 = DoubleConv(block_size[3] * 2, block_size[3])
        self.C8 = DoubleConv(block_size[2] * 3, block_size[2])
        # self.C9 = DoubleConv(block_size[1] * 4, block_size[1])
        # self.C10 = DoubleConv(block_size[0] * 5, block_size[0])
    ...
    def forward(self, x: torch.Tensor) -> Tuple[Any, Any, Any, Any]:
        ...
        x1 = self.up11(x1)  # [384,28,28]

        x2 = self.up22(x1)  # [192,56,56]
        x3 = self.up33(x2)  # [96,112,112]
        x4 = self.up44(x3)
        # convnext_downsample
        # x_ = self.upsample(x)
        y3 = self.convnext.downsample_layers[0](x)
        ...
        # d0_1 = self.C1(torch.cat([d0_1, d0_0], dim=1))
        # q1 = self.c1(d0_1)
        # d0_1 = q1 * d0_1
        ...
        d2_0 = z2
        # d1_1 = self.u33_(d2_0)
        # d1_1 = self.C2(torch.cat([d1_1, d1_0], dim=1))
        # d0_2 = self.u44_(d1_1)
        # # d0_2 = d0_2 + d0_1 + d0_0
        # d0_2 = self.C3(torch.cat([d0_2, d0_1, d0_0], dim=1))
        # q2 = self.c2(d0_2)
        # d0_2 = q2 * d0_2
        # out2 = self.ouu_(d0_2)
        ...
        d2_1 = self.u2_(d3_0)
        # d2_1 = d2_1 + d2_0
        d2_1 = self.C4(torch.cat([d2_1, d2_0], dim=1))
        d1_2 = self.u3_(d2_1)
        # d1_2 = d1_2 + d1_1 + d1_0
        # d1_2 = self.C5(torch.cat([d1_2, d1_1, d1_0], dim=1))
        d1_2 = self.C55(torch.cat([d1_2, d1_0],dim=1))
        d0_3 = self.u4_(d1_2)
        # d0_3 = d0_3 + d0_2 + d0_1 +d0_0
        # d0_3 = self.C6(torch.cat([d0_3, d0_2, d0_1, d0_0], dim=1))
        d0_3 = self.C66(torch.cat([d0_3, d0_0],dim=1))
        q3 = self.c3(d0_3)
        d0_3 = q3 * d0_3
        out3 = self.ou_(d0_3)
        ...
        # d3_1 = self.uf(d4_0)
        # # d3_1 = d3_1 + d3_0
        # d3_1 = self.C7(torch.cat([d3_1, d3_0], dim=1))
        # d2_2 = self.u1(d3_1)   #[384,28,28]
        # # d2_2 = d2_2 + d2_1 + d2_0
        # d2_2 = self.C8(torch.cat([d2_2, d2_1, d2_0], dim=1))
        # d1_3 = self.u2(d2_2)
        # # d1_3 = d1_3 + d1_2 + d1_1 + d1_0
        # d1_3 = self.C9(torch.cat([d1_3, d1_2, d1_1, d1_0],dim=1))
        # d0_4 = self.u3(d1_3)
        # # d0_4 = d0_4 + d0_3 + d0_2 + d0_1 + d0_0
        # d0_4 = self.C10(torch.cat([d0_4, d0_3, d0_2, d0_1, d0_0], dim=1))
        # q4 = self.c4(d0_4)
        # d0_4 = q4 * d0_4
        # out4 = self.ou(d0_4)
